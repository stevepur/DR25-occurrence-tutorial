...
def midpoint_transit_depth(rstar, rp, limbcoeffs):
    """Provides exact central depth of transit [ppm] for a midpoint cross (e.g. b=0)
...
class kepler_detection_efficiency_model_data:
    ...
    def __init__(self, filePath=''):
        ...
        # This is an initial 2D detection efficiency as a function of MES and stellar radius
        # in the text table

        # load as a flat table with 3 columns
        ...
        # baseData is the initial detection efficiency
        baseData = dataBlock[:,2].ravel()
        # reshape into a 2D table, with dimensions required by the table
        nMes = 27     # DO NOT CHANGE
        nRs = 11     # DO NOT CHANGE
        ...
        #   Mes and Rstar.  See Fig 3 of the KSCI to view this data
        self.baseDetFunc = interp.RectBivariateSpline(xMes, yRs, baseData, bbox=[2.5, 30.0, 0.1, 1.25], kx=1, ky=1)
        self.rStarRange = np.array([0.1, 1.25]) # enforces that DE for R>1.25 = DE at R=1.25
        
        # Read in the CDPP Slope plane correction for DE model on the 3D space (MES, slpCDPPl, slpCDPPs)
        ...
        # See Fig 2 of the KSCI to see where slpCDPPl, slpCDPPs is defined
        nMes = 27     # DO NOT CHANGE
        nSL = 10     # DO NOT CHANGE
        nSS = 13     # DO NOT CHANGE
        ...
        #  Mes, CdppSlope Long, and CdppSlope Short.  See Fig 4 of the KSCI to view this data
        ...
        # Define the period and MES ranges for the coefficients.
        ...
        # Define the coefficients and model order
    ...
    def generate_detection_eff_grid(self, rstar, cSlopeL, cSlopeS, dutyCycle, \
        ...
        # There is a DE model for the 5 period bins (see section 3.4 of the KSCI)
        self.detEffFunctions = []
        self.detEffSmearFunctions = []

        # Our MES is a single value but we need to account for varying impact parameter
        # by computing the DE as an average weighted by the MES distribution at a
        # given (period, planet radius).  This distribution is given by a beta dist. fit.
        # Get the beta distribution mes smearing parameters (see sec 4 of the KSCI)
        self = self.mes_smearing_parameters(cSlopeL)
        # Get a random set of beta distribution samples to perform the weighted average
        ...
        # compute the expected number of transits for this data span and duty cycle
        useNtran = dataSpan / self.midPers * dutyCycle
        # print("nPeriods = " + str(nPeriods) + ", nValues = " + str(nValues))

        # compute the actual smeared detection efficiency in the five period bins, including vetting efficiency
        for ii in range(nPeriods): # 5 bins, see sec 3.4 of the KSCI
            for jj in range(nValues): # number of MES bins, 
                if self.fitOrder[ii, jj] == 1: # happens at high MES and periods where the model is complete
                    # compute base DE + correction, see sec 3.4 of the KSCI
                    self.detEffValues[ii, jj] = self.fitCoeffs[ii, jj, 2] + \
                            detModel[jj]
                else:
                    # compute DE, see eqn 3 in the KSCI 
                    cfs = self.fitCoeffs[ii, jj, :]
                    self.detEffValues[ii, jj] = cfs[0]*useNtran[ii] + \
                        cfs[1]*detModelSLSS[jj] + \
                        cfs[2] + \
                        detModel[jj]
            # We've created a discrete DE model as a function of MES for this period bin
            ...
            # this is to save the unsmeared DE just in case
            self.detEffFunctions.append(detEffFunc)


            # Smear DE model by doing monte carlo weighted average using the points from the beta distribution
            useSmearDetEffValues = np.zeros_like(useDetEffValues)
            # The mesSim range from 0 to 1, with 1 being identity so is at impact parameter 0
            for jj in range(len(useMesValues)):
                mesRan = mesSim * useMesValues[jj]
                useSmearDetEffValues[jj] = np.mean(detEffFunc(mesRan)) # weighted average
            ...
            # The result is a 1D DE model as a function of MES for this period bin.  See fig. 7 in the KSCI.
        return self
    ...
    def final_detEffs(self, mes, per):
        ...
        # get the period bin index for the desired periods
...
def kepler_single_comp_dr25(data, DEMod=None):
    ...
    # and fill in the window and 1sigma depth function arrays
    plan_det_met = tps_planet_detection_metrics(data.id, \
                         data.planet_detection_metric_path, \
                         want_wf=True,want_osd=True)
        
    # Calculate transit duration in hours along period_want list
    # .period_want is the period grid, set by the calling function
    transit_duration_1d = transit_duration_zero(data.rstar,
                                           data.logg,
                                           data.period_want,
                                           data.ecc)
    # To avoid extrapolation force the transit duration to be
    #  within the allowed range.  This clamps to max/min(pulsedurations)
    ...
    #  transit_duration_1d is the expected transit duration as a function of period_want
    #  here we map transit durations into an index into pulse durations
    #  so given a transit duration we can look up the appropriate pulse duration
    pulse_index_1d = np.digitize(transit_duration_1d, pulsedurations)
    #  trim to legal indices
    pulse_index_1d = np.where(pulse_index_1d > 0, pulse_index_1d - 1, pulse_index_1d)

    # The one-sigma depth function provides the depth that results in a 1MES detection
    # as a function of period and duration

    # The window function provides the probability of >= 3 transits
    # as a function of period and duration
    # (the window function is averaged over phase)
    ...
    # We want evaluate the window and 1 sigma depth functions for 
    # arbitrary period and duration.
    # We do this via bilinear interpolation implemented as an explicit
    # interpolation of two linear interpolations in duration.
    # We use a separate interpolant for each pulse duration bin.
    # So we set up a 1 sigma depth function interpolation 
    # and window function interpolation for each pulse duration.

    # For efficiency, only work with the pulse durations that are requested by pulse_index_1d

    # The result is one_sigma_depth_1d and windowfunc_1d, giving the 1 sigma depth and 
    # window functions at the requested periods on this target star 
    ...
    # Calculate limb-darkened transit depth at mid-transit with impact parameter=0
    # for this star and requested planet sizes
    depth_1d = midpoint_transit_depth(data.rstar, data.rp_want, \
                        data.limbcoeffs)

    # Now ready to make things 2d
    # Up to now we've been treating planet period and radius independently
    # We use as inputs to predict MES as a function of period and radius

    # set up the 2D arrays replicating the depth(radius) into depth(radius, period)
    # by replicating depth(radius) for each period.
    # Do the same for 1 sigma depth function(period) by replicating for each radius.
    # And so on for period, windowfunc_1d and probtransit_1d
    ...
    #  compute the detection efficiencies without the window function
